...
# ---- A15/A16: byte-identity against the PRE-Phase-2 baseline (origin/main)
def test_flags_off_is_byte_identical_to_pre_phase2_golden():
    ...
    Guaranteed byte-identical: the prompt parts, the wire schema, and the
    entire result except two documented additive plan keys (`loops`,
    `dialogueAdjustments`) which exist on the new model with empty defaults.
    Those two keys are asserted to be exactly [] with all flags off."""
...
# ---- A14: quotes spanning a sentence boundary resolve
